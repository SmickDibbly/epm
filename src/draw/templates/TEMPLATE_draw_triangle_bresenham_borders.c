...
// the follow are just to shut up the editor
void compute_xspans(Window *win, zgl_Pixel v0, zgl_Pixel v1, zgl_Pixel v2, int32_t *min_y, int32_t *max_y);
extern uint32_t g_xspans[];
//#error
#define ORTHO
#define TEXEL
//#define LIGHT_FLAT
#define LIGHT_GOURAUD
#define COUNT
#define HILIT
#endif
...
uint32_t DRAWTRI_FUNCTION(Window *win, ScreenTriangle *tri) {
    Fix64Vec_2D
    ...
    /* NOTE: As of 2023-06-27, the compute_xspans function already returns min_y and max_x clipped to window.
...
#  endif
    ...
    uint64_t flat_bri = tri->brightness;
#endif
...
#if (defined LIGHT_FLAT) || (defined LIGHT_GOURAUD)
                color = (uint32_t)
                    ((((color & 0x00FF00FFu) * flat_bri) & 0xFF00FF00u) |
                     (((color & 0x0000FF00u) * flat_bri) & 0x00FF0000u)) >> 8;
#endif
#if (defined LIGHT_GOURAUD)
                color = (uint32_t)
