    ...
    for (line = CRT_TOP; line < CRT_BOT; line++) {
        ...
        for (pos = scanL; (int)pos < scanR && cL < cR; pos += dx) {
            ...
            switch (v->out_format) {
                case CRT_PIX_FORMAT_RGB:
                case CRT_PIX_FORMAT_RGBA:
                    cL[0] = (unsigned char)(bb >> 16 & 0xff);
                    cL[1] = (unsigned char)(bb >>  8 & 0xff);
                    cL[2] = (unsigned char)(bb >>  0 & 0xff);
                    break;
                case CRT_PIX_FORMAT_BGR: 
                case CRT_PIX_FORMAT_BGRA:
                    cL[0] = (unsigned char)(bb >>  0 & 0xff);
                    cL[1] = (unsigned char)(bb >>  8 & 0xff);
                    cL[2] = (unsigned char)(bb >> 16 & 0xff);
                    break;
                case CRT_PIX_FORMAT_ARGB:
                    cL[1] = (unsigned char)(bb >> 16 & 0xff);
                    cL[2] = (unsigned char)(bb >>  8 & 0xff);
                    cL[3] = (unsigned char)(bb >>  0 & 0xff);
                    break;
                case CRT_PIX_FORMAT_ABGR:
                    cL[1] = (unsigned char)(bb >>  0 & 0xff);
                    cL[2] = (unsigned char)(bb >>  8 & 0xff);
                    cL[3] = (unsigned char)(bb >> 16 & 0xff);
                    break;
                default:
                    break;
            }
