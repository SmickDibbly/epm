...
crt_modulate(struct CRT *v, struct NTSC_SETTINGS *s)
{
    ...
    int iccf[4];
    int ccmod[4]; /* color phase for mod */
...
#if (CRT_CHROMA_PATTERN == 1)
                cb = ccburst[(t + inv_phase * 2) & 3];
#else
                cb = ccburst[t & 3];
#endif
                line[t] = (signed char)(BLANK_LEVEL + (cb * BURST_LEVEL)) >> 5;
                iccf[t & 3] = line[t];
            }
        }
    }
    ...
    for (y = 0; y < desth; y++) {
        ...
        for (x = 0; x < destw; x++) {
            ...
            /* bandlimit Y,I,Q */
            fy = iirf(&iirY, fy);
            fi = iirf(&iirI, fi) * ph * ccmod[(x + 0) & 3] >> 4;
            fq = iirf(&iirQ, fq) * ph * ccmod[(x + 3) & 3] >> 4;
            ire += (fy + fi + fq) * (WHITE_LEVEL * v->white_point / 100) >> 10;
            if (ire < 0)   ire = 0;
            if (ire > 110) ire = 110;

            v->analog[(x + xo) + (y + yo) * CRT_HRES] = (signed char)ire;
        }
    }
    for (x = 0; x < 4; x++) {
        for (n = 0; n < 4; n++) {
